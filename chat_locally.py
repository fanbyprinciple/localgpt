...
#https://gist.github.com/abhishekkrthakur/401c39d422fb6beff1600effe81f498a
...
# docker run --gpus 0 -p /data:/data -e LOG_LEVEL=info, text_generation_router=debug ghcr.io/huggingface/text-generation-inference:0.9.1 --model-id OpenAssistant/falcon-7b-sft-top1-696 --num-shard 1
...
def embed(file_n, window_size, step_size):
    text = extract_text(file_n)
...
def search(query, model, cross_encoder, embeddings, pragraphs, top_k):
    query_embedding = model.encode(query, conver_to_tensor=True)
    query_embeddings = query_embeddings.cuda()
    hits = util.semantic_search(query_embeddings, embeddings, top_k=top_k)[0]
    
    cross_input = [[query, paragraphs[hits["corpus_id"]]] for hit in hits]
    ...
    scores = cross_encoder.predict(
        [(query, p) for p in paragraphs]
    )

if __name__ == "__main__":
    args = parse_args()
    model, cross_encoder, embeddings, paragraphs = embed(args.file, args.window_size, args.step_size)
